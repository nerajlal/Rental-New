...
<section class="hero-section" style="background: linear-gradient(135deg, {{ $bgColor }} 0%, #00a876 100%); color: white; padding: 80px 0; text-align: center;">
    <div class="container">
        <h1 class="display-4 fw-bold mb-4" style="font-size: 48px;">{{ $heading }}</h1>
        <p class="lead mb-4" style="font-size: 20px; opacity: 0.95; max-width: 600px; margin: 0 auto 30px;">
            {{ $subheading }}
        </p>
        <a href="{{ $buttonUrl }}" class="btn btn-lg" style="background: white; color: {{ $bgColor }}; border: none; padding: 12px 32px; font-weight: 600; border-radius: 6px; text-decoration: none; display: inline-block;">
            {{ $buttonText }}
        </a>
    </div>
</section>
